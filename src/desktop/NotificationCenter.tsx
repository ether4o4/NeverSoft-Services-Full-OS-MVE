 ...
 *   • open notifications (driven by the intent registry);
...
import { ActionRegistry, Intent } from '../mve/ActionRegistry';
...
const NotificationCenter: React.FC<{ visible: boolean; onClose: () => void }> = ({
  ...
  useEffect(
    () => ActionRegistry.subscribe(() => setIntents(ActionRegistry.open())),
    [],
  );
          ...
          <ScrollView
            ...
            <View style={styles.notifBox}>
              ...
              {intents.length === 0 ? (
                ...
                intents.map(it => (
                  <View key={it.id} style={styles.notifRow}>
                    <Text style={styles.notifVerb}>{it.verb}</Text>
